...
class PaletteGradientBar(QtWidgets.QWidget):
    ...
    def _collect_stops(self, /):
        ...
        pts_get = getattr(self._spline, "points", None)
        pts = pts_get() if callable(pts_get) else getattr(self._spline, "_points", None)
        pts = pts or []

        col_get = getattr(self._spline, "point_colors", None)
        if callable(col_get):
            try:
                cols = col_get()
            except Exception:
        ...
        closed = bool(getattr(self._spline, "_closed", False))
